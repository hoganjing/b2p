...
通用、无书专用硬编码：专有名词白名单从「每本书的 brief」读取，默认宽松。
...
def _load_white(args):
    """从 briefs/<slug>.yaml 或命令行载入白名单。"""
    white = set()
    if args.white:
...
def main():
    ...
    ap.add_argument("--book", help="briefs/<slug>.yaml 的书名 slug，载入专有名词白名单")
    ...
    for f in files:
        ...
        if stray:
            print("   STRAY:", stray); bad = True
    if bad and not args.no_gate:
        print("\n❌ 门禁未过：noise=0 且 strayEN=0 才允许交付。", file=sys.stderr)
        return 1
    print("\n✅ 门禁通过（noise=0, strayEN=0）。")
    return 0
